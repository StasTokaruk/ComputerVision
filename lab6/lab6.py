    ...
    def detect_vehicles(self, frame):
        ...
        for i in range(detections.shape[2]):
            ...
            if confidence > self.conf_threshold:
                ...
                if label_name in self.VEHICLE_CLASSES:
                    vehicle_count += 1
...
if __name__ == "__main__":
    ...
    ap.add_argument("-c", "--confidence", type=float, default=0.4, help="мінімальний поріг достовірності")
    ap.add_argument("-v", "--video", default="Video_1.mp4",help=" шлях до відеофайлу або камера (0)")
    ...
    while True:
        ...
        frame, count = detector.detect_vehicles(frame)

        current_time = time.time()
        fps = 1 / (current_time - prev_time)
        prev_time = current_time

        cv2.putText(frame, f"Vehicles: {count}", (20, 30),cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
